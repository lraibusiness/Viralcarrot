  ...
  try {
    ...
    if (!mainFood || !mainFood.trim()) {
      ...
      return NextResponse.json(
        { success: false, error: 'Please select a main food item' },
        { status: 400 }
      );
    }
    ...
    // Recipe database organized by main food items
    ...
    // Get recipes for the specific main food item
    let matchedRecipes = recipeDatabase[mainFood.toLowerCase()] || [];

    console.log('API: Found recipes for', mainFood, ':', matchedRecipes.length);

    // If no specific recipes found, try to find similar main foods
    if (matchedRecipes.length === 0) {
      ...
      const similarFood = similarFoods[mainFood.toLowerCase()];
      if (similarFood && recipeDatabase[similarFood]) {
        matchedRecipes = recipeDatabase[similarFood];
        console.log('API: Using similar food', similarFood, 'for', mainFood);
      }
    }

    // If still no recipes, return a generic message
    if (matchedRecipes.length === 0) {
      ...
      return NextResponse.json({
        ...
        recipes: [],
        total: 0,
        message: `No recipes found for ${mainFood}. Try selecting a different main food item.`
      });
    }
